    ...
    <div x-data="newArtist" class="flex container relative p-7 flex-col gap-2 items-end">
        ...
        <div class="w-full h-auto flex items-center justify-between">
            ...
            <form action="{{route('list.styles')}}">
                ...
                <div class="flex md:w-48 w-32 h-full">
                    ...
                    <input type="text"  name="style" placeholder="نام سبك"
        ...
        <div x-show="open" x-cloak
            ...
            <div class="w-full flex justify-end items-center h-7 border-b border-gray-300 ">
                <h3 class="text-sm font-normal text-gray-500">اضافه كردن سبك</h3>
            </div>
            <form dir="rtl" action="{{route('store.styles')}}" method="post" class="w-full flex relative flex-col gap-2 justify-center items-center ">
                @csrf
                <!-- title -->
                <input type="text" name="title" placeholder="عنوان سبك" class="w-full md:w-1/3 h-8 border border-gray-300 focus:ring-0 focus:border-gray-300 text-xs font-normal  text-gray-500 rounded-lg">
                <!-- button -->
                ...
                <tbody>
                    @foreach($styles as $style)
                        <tr class="text-xs odd:bg-white even:bg-gray-100">
                            <td class="p-2  ">{{$style->id}}</td>
                            <td class="p-2  ">{{$style->title}}</td>
                            <td class="p-2 flex gap-2">
                                <form method="post" action="{{route('delete.style', $style->slug)}}">
                                    @csrf
                                    @method('DELETE')
                                    <button title="حذف"
                                ...
                                <a href="{{route('edit.style', $style->slug)}}" title="ويرايش"
